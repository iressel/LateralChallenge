...
- [x] **T008 — Establish SQL Server integration infrastructure and verify the relational schema**
  ...
  - **Completion evidence (2026-08-02):** On Docker `linux/amd64`, Testcontainers 4.13 used pinned SQL Server image `mcr.microsoft.com/mssql/server:2022-CU26-ubuntu-22.04@sha256:ba4c8329f48fb8f02e1416be6a930ebfd71268caee78aa985f3af4315e457c89`, reporting product version `16.0.4265.3` and `Developer Edition (64-bit)`. Each run created a unique clean database, applied only `20260802142305_InitialCmsPersistence`, and proved exactly four application tables, separate required active `datetime2(7)` timestamps, physical schema metadata, all 18 checks, and three non-primary indexes. Direct SQL covered all 27 required constraint/rowversion scenarios using errors 547, 2601, and 2627; production DI resolved scoped SQL Server read/write contexts without startup migration; the read context queried NoTracking and rejected all four SaveChanges overloads; and a `db_datareader` login selected successfully while INSERT/UPDATE/DELETE failed with 229 and schema creation with 262. Release build passed with 0 warnings/errors, unit tests passed 239/239, the Model/Migration filter passed 30/30, and two consecutive SQL Server runs passed 20/20 each with 0 failed or skipped; format/diff checks passed, no container remained, no migration/model/package changed, and T009 was not started.
...
- [x] **T009 — Implement and prove transactional event processing on SQL Server**
  ...
  - **Completion evidence (2026-08-02):** `CmsEventBatchService` and the application-owned `IEventTransactionExecutor` were implemented with `SqlServerEventTransactionExecutor`, sequential per-position execution, one explicit Serializable transaction inside the SQL Server execution-strategy delegate, bounded three-attempt handling for application concurrency/dependency failures, and transaction-owned `sp_getapplock` resources derived from a bounded namespace plus SHA-256 of the exact UTF-8 EntityId. Durable `BatchId + Sequence` recovery proved safe ambiguous-commit handling; exact external/derived replay, EventId content conflict, invalid/conflict outcome preservation, and payload-too-large invalid identity ownership use only the existing canonical identity contract. Production processing proved every state operation, immutable revisions, delete/recreation, generation exhaustion, cancellation and prior-commit preservation, plus exact AC-053/AC-057 high-watermark behavior. Ten batch-service tests, 8 coordinated concurrency scenarios, transient retry, post-commit ambiguity, lock ownership/release/timeout, atomic rollback, and production DI all passed against pinned SQL Server image `mcr.microsoft.com/mssql/server:2022-CU26-ubuntu-22.04@sha256:ba4c8329f48fb8f02e1416be6a930ebfd71268caee78aa985f3af4315e457c89` (`16.0.4265.3`, `Developer Edition (64-bit)`). Release build passed with 0 warnings/errors; batch tests passed 10/10, all unit tests 251/251, Model/Migration 30/30, frozen SQL Server coverage 20/20, two consecutive EventProcessing runs 41/41 in 32.9003 and 32.3765 seconds, and the complete integration project 88/88, all with 0 failed or skipped. Format/diff/security checks passed, no container remained, no migration/model/schema/package/project changed, and T010 was not started.
  - **Suggested commit boundary:** Production transactional event-processing flow and SQL Server integration proof.

## 9. Basic Authentication

- [x] **T010 — Implement isolated CMS and consumer Basic schemes and policies**
  ...
  - **Completion evidence (2026-08-02):** Added isolated `CmsBasic` and `ConsumerBasic` schemes; explicit `CmsEvents`, `ConsumerAccess`, and `AdministratorAccess` policies; and exactly the `CmsService`, `NormalConsumer`, and `Administrator` roles bound from `Authentication:Credentials`. `ValidateOnStart` and direct/production-host tests cover missing identities and fields, safe username syntax and the centralized maximum, CMS username lengths 9/10/20/21, exact GUID `D` password format including rejection of surrounding whitespace, and ordinal-distinct usernames/GUID-distinct passwords including case-different text for one GUID. Credential pairs use length-prefixed strict UTF-8 representations, SHA-256 equal-length digests, and `CryptographicOperations.FixedTimeEquals`; Basic parsing bounds encoded and decoded input, rejects malformed/strict-UTF-8 failures without 500, and clears decoded byte/character and digest buffers. Real middleware proves exact `Basic realm="CmsBasic"` and `Basic realm="ConsumerBasic"` challenges, cross-scheme 401 isolation, normal-consumer administrator denial as 403 without challenge, `no-store` on 401/403 and non-Development HTTPS redirects, runtime-only test credentials, captured message/state/scope/exception leak protection, and production Program/DI/middleware startup with no production route. Two final Authentication runs passed 53/53 in 2.3508 and 1.7432 seconds; unit tests passed 251/251, Model/Migration 30/30, `Category=SqlServer` 20/20, `Category=EventProcessing` 41/41, and the complete integration project 141/141, all with 0 failed and 0 skipped. Release build passed with 0 warnings/errors; restore used only NuGet.org; tool restore, full format, staged diff, and security inspections passed; no SQL Server container remained; no credential/configuration leak, migration/model/schema/package-version change, or public endpoint was introduced; and T011 was not started.
...
- [x] **T014 — Add safe operational telemetry, health, error handling, and cache/TLS protections**
  ...
  - **Completion evidence (2026-08-03):** Added allowlisted structured batch/event/request logs, platform-native bounded metrics for outcomes/codes/latency/SQL retries/deadlocks/failures/authentication failures, safe correlation and Activity trace propagation, one global Problem Details boundary, unified no-store response protection, non-Development HTTPS ordering, safe connection/credential startup validation, and anonymous minimal live/ready health endpoints with separate bounded write/read SQL probes. Correlation metadata now remains separate from the ASP.NET Core request identifier, and request-completion logging observes the final status emitted by the global exception handler. Real HTTP/authentication/SQL Server tests include payload/credential/header/connection/exception leak sentinels, all deterministic outcomes, final 500/503 completion-log status, a real EF-retried deadlock, SQL failure metrics, liveness without SQL, dependency-specific readiness, cancellation, existing read-context guards, and the runtime-generated SELECT-only login rejecting writes. Release build passed with 0 warnings/errors; two final Observability/Security/Health runs passed 37/37 in 29 and 25 seconds; unit tests passed 251/251 in 471 ms; and the complete integration project passed 268/268 in 52 seconds, all with 0 failed or skipped. Full format, diff, scope, leakage, and container-cleanup checks passed; no migration/model/schema/package/project change was introduced; no container remained; and T015 was not started.
  - **Suggested commit boundary:** Operational and security hardening.

## 14. Docker and cross-platform setup

- [x] **T015 — Add reproducible SQL Server/API local setup and Apple Silicon alternative**
  ...
  - **Completion evidence (2026-08-03):** Added the supported x86-64 Compose chain `sql` → idempotent database/principal initialization → dedicated EF migration → non-root API readiness, with SQL Server pinned exactly to `mcr.microsoft.com/mssql/server:2022-CU26-ubuntu-22.04@sha256:ba4c8329f48fb8f02e1416be6a930ebfd71268caee78aa985f3af4315e457c89`. Runtime-only distinct SA/migration/write/read passwords and exact Basic-auth keys are mandatory; the migration target generates the idempotent script with repository-local `dotnet-ef` 10.0.10 and applies it as the migration principal, while the API uses schema-DML write and SELECT-only read principals. ContainerSetup tests passed 14/14 in 86 ms; the `pwsh` clean-volume smoke passed in 30.20 seconds with live/ready 200, exactly one expected migration, authenticated consumer read, rejected read-principal INSERT/UPDATE/DELETE/schema creation, and a persisted real webhook write. Category=SqlServer passed 20/20 in 2 seconds, unit tests passed 251/251 in 337 ms, and the complete integration suite passed 282/282 in 48 seconds, all with 0 failed or skipped. Release build passed with 0 warnings/errors; format, diff, credential/header/connection/image/LocalDB/environment, scope, and cleanup checks passed; no migration/model/schema/package/project changed; no container or volume remained; Apple Silicon guidance uses remote SQL Server/Azure SQL without emulation; and T016 was not started.
...
- [x] **T017 — Document setup, contracts, retry semantics, security, and platform limits**
  ...
  - **Completion evidence (2026-08-05):** Added root `README.md` with 19 numbered sections covering local and remote setup, required configuration keys, pinned SQL image constraints, raw-array webhook examples using wire `id`, documented event-type casing normalization examples, response/outcome/code contracts, whole-request retry semantics, role-based visibility, health behavior, timestamp/high-watermark semantics (including the AC-057 sequence), security boundaries, and explicit assumptions/unresolved external questions. Added `tests/CmsSync.IntegrationTests/Documentation/DocumentationArtifactTests.cs` (`Category=Documentation`) to verify README existence/section structure, markdown link/path validity, command/pin coverage, webhook-example JSON contract (raw array, wire `id`, casing variants, delete applicability), outcome/retry/timestamp wording, security/platform statements, and that T018 remains unchecked. Validation passed with `./scripts/validate-repository-policy.ps1`, `dotnet restore LateralChallenge.sln --source https://api.nuget.org/v3/index.json`, `dotnet format LateralChallenge.sln --verify-no-changes --no-restore`, `dotnet build LateralChallenge.sln --configuration Release --no-restore`, `dotnet test tests/CmsSync.IntegrationTests/CmsSync.IntegrationTests.csproj --configuration Release --no-build --no-restore --filter "Category=Documentation"` (8/8), `./scripts/validate-container-setup.ps1` (clean-volume Compose, migration, read-only probe, health, and webhook smoke all passed), and `git diff --check`; no production C# behavior, migrations, CI workflow, or container policy artifacts were changed, and T018 was not started.
